...
# 引入benepar分析器
nlp = spacy.load('en_core_web_md')  # 英文
if spacy.__version__.startswith('2'):
    nlp.add_pipe(benepar.BeneparComponent("benepar_en3"))
else:  # 走这个
    nlp.add_pipe("benepar", config={"model": "benepar_en3"})  # 英文
    ...
    for sent in list(doc.sents):
        ...
        for _ in sent_res:
            ...
            if len(sent_res) == 1:  # 如果只有一个单词
                ...
                for __ in _:
                    # 从所有划分好结构的句子中去除从句级结构 or 超过5个词的短语 or 只有一个结构的短语
                    if __[0] in sent_tag or len(__[1].split(' ')) > max_phrase_len or len(_) == 1:
                        flag = True
                        break
